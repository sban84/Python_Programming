...
words_with_count = rdd.flatMap(lambda x: x.split(" ")).map(lambda x: (x, 1)).reduceByKey(lambda x, y: x + y).sortBy(
...
# order by  multiple cols , if first col has tie then sec order by will happen by sec col according to the ascending rules

df = df.groupBy(col("word")).count().alias("count").orderBy([col("count"), col("word")], ascending=[False, False])
df.show()

# just how to use selectExpr
df.selectExpr("length(word) as len_word").show()
